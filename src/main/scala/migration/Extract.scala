...
object Extract {
  ...
  case class ValidationResult(zipFile: Path, errorMessages: Seq[String]) {
    def desc: String = errorMessages match {
      case Seq() => "OK"
      case errors =>
        errors.mkString(s"failed w/ ${errors.size} error(s):\n\t", "\n\t", "\n")
    }
  }

  case class ZipEntryResult(entryZipFileName: String,
                            index: Int,
                            result: Either[Throwable, ValidationResult]) {
    def header =
      s"${index.toString.reverse.padTo(4, '0').reverse}: $entryZipFileName"
  ...
  private def prepareFinalReport(
      either: Either[Throwable, List[ZipEntryResult]]) = either match {
    case Left(err) =>
      val result = putStrLn(s"Extraction failed with ${err}") *> Task.fail(err)
      result.unit
    case Right(results) =>
      val outputs: ZIO[Console, Nothing, Unit] = ZIO
        .foreach(results) { result =>
          putStrLn(result.desc)
        }
        .unit
      val isSuccess: ZIO[Any, Throwable, List[Boolean]] = ZIO.foreach(results) {
        result =>
          ZIO(result.success)
      }

      val finalTask: ZIO[Any, Throwable, Unit] = isSuccess.flatMap {
        successes =>
  ...
  def extractAndValidate(
      config: ParsedConfig): ZIO[Console, Throwable, List[ZipEntryResult]] = {
    import config._
    ...
    for {
      ...
      forks <- ZIO.foreach(zipFilesNames.zipWithIndex) {
        case (zipEntry, index) =>
          val forked: URIO[
            Console,
            Fiber.Runtime[Nothing, Either[Throwable, ValidationResult]]] =
            extractAndValidateEntry(config.url,
                                    downloadDirectory,
                                    dataDirectory,
                                    index,
                                    zipEntry,
  ...
  private def extractAndValidateEntry(
      url: String,
