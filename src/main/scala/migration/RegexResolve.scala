...
object RegexResolve {
      ...
      if (index < 0) {
        ...
        try {
          ...
          case _: ArrayIndexOutOfBoundsException => {
            sys.error(s"regex only captured ${groups.size} groups, so cannot replace index $index in pattern '${replacementPattern}', captured group: ${groups.mkString("[", ",", "]")}")
          }
        }
      }
    }
  ...
  object Group {
    def apply(parts: String*) = new Group(parts.mkString(""), parts.toIndexedSeq)
  }
    ...
    replacementPattern match {
      case BackRef(before, indexStr, after) =>
        ...
        createReplacement(after) match {
          case Left(finished) =>
            val swap = (group: Group) => s"$before${group.get(replacementPattern, index)}$finished"
            Right(swap)
          case Right(continuation) =>
            val swap = (group: Group) => s"$before${group.get(replacementPattern, index)}${continuation(group)}"
            Right(swap)
        }
